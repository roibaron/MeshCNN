...
class MeshConv2(MeshConv):
    def __init__(self, in_channels, out_channels, fc_dim, firstLayer=False, k=5, bias=True):
        super(MeshConv2, self).__init__(in_channels, out_channels, k, bias)
        self.firstLayer = firstLayer
        self.fc = nn.Linear(fc_dim[0], fc_dim[1])
...
class MeshConvNet(nn.Module):
    ...
    def __init__(self, norm_layer, nf0, conv_res, nclasses, input_res, pool_res, fc_n,
        ...
        self.k_list = [(5,5) , (5,16), (5,32), (5,32)]

        for i, ki in enumerate(self.k[:-1]):
            setattr(self, 'conv{}'.format(i), MResConv(ki, self.k[i + 1], self.k_list[i], i, nresblocks))
    ...
    def forward(self, x, mesh):
        ...
        for i in range(len(self.k) - 1):
            ...
            #print(str(i)+" after conv "+str(x.size()))
            x = F.relu(getattr(self, 'norm{}'.format(i))(x))
            #print(str(i)+" after relu "+str(x.size()))
            x = getattr(self, 'pool{}'.format(i))(x, mesh)
            #print(str(i)+" after pool "+str(x.size()))
...
class MResConv(nn.Module):
    def __init__(self, in_channels, out_channels, fc_dim, i, skips=1):
        ...
        self.fc_dim = fc_dim
        if (i==0):
            self.conv0 = MeshConv2(self.in_channels, self.out_channels, (5,5), k=5, firstLayer=True, bias=False)
        else:
            self.conv0 = MeshConv2(self.in_channels, self.out_channels, (fc_dim[0], fc_dim[0]), k=self.fc_dim[0], bias=False)
            
        for i in range(self.skips):
            setattr(self, 'bn{}'.format(i + 1), nn.BatchNorm2d(self.out_channels))
            setattr(self, 'conv{}'.format(i + 1),
                    MeshConv2(self.out_channels, self.out_channels, fc_dim, k=self.fc_dim[1], bias=False))

    def forward(self, x, mesh):
        #print("AAA: x: "+str(x.shape)+" fc_dim: "+str(self.fc_dim))
        x = self.conv0(x, mesh)
        x1 = x
        for i in range(self.skips):
